...
def parse_domain(domain: str):
  ...
  if matched := re.compile("^(.*)_\\((.*)\\)$").match(domain):
    domain = matched.group(1) if len(matched.group(1)) > len(matched.group(2)) else matched.group(2)
    ...
    for line in f:
        for item in parse_line(clean_line(line)):
          for iitem in parse_line(item):
            domains += parse_line(iitem) # iterate two levels to ensure all nested domains are extracted
...
def main():
  ...
  open(pathlib.PurePath(ROOT_DIR, "../data/cleaned"), 'w').write("\n".join(str(i) for i in domains)) 
